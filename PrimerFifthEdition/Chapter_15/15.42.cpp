...
TextQuery::TextQuery(std::ifstream& is) : file(new std::vector<std::string>)
{
    ...
    {
        for (auto comma_pos = std::find(text.begin(), text.end(), '.'), after_last_comma = text.begin();
             after_last_comma < comma_pos;
             comma_pos = std::find(after_last_comma, text.end(), '.'))
        {
            file->emplace_back(std::string(after_last_comma, comma_pos + 1));
            if (comma_pos == text.end() - 1)
            {
                break;
            }
            after_last_comma = comma_pos + 1;
        }
